export default async function handler(req, res) {
  ...
  try {
    const { message, history = [], image, audio, tone = 'friendly' } = req.body || {}

    if (!message && !image && !audio) {
      return res.status(400).json({ error: 'Сообщение обязательно.' })
    }

    let toneInstruction = 'Будь вежливым, добрым и веселым.'
    if (tone === 'agronomist') {
      toneInstruction = 'Ты — опытный агроном и ученый-биолог. Давай профессиональные, научно обоснованные агротехнические советы по уходу за культурами, составам удобрений, болезням и почве.'
    } else if (tone === 'chef') {
      toneInstruction = 'Ты — шеф-повар ресторанного уровня. Подсказывай кулинарные секреты, тонкости термообработки, идеальные специи и рецепты заготовок.'
    } else if (tone === 'concise') {
      toneInstruction = 'Отвечай предельно кратко, тезисно, списком в 2-4 пункта, строго по делу, без вводных слов и воды.'
    }
...
Твой стиль общения: ${toneInstruction}

📋 Правила общения:
1. Четкость и структурированность: Отвечай по существу. Используй списки, **жирный текст** и абзацы.
2. Тон: ${toneInstruction}
